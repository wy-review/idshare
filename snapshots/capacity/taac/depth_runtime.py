#!/usr/bin/env python3
"""Three-seed extension of the frozen R01 depth runtime."""
...
def config(p, run, output, local=False, train_only=False):
    ...
    assert run['method'] in ['continuous', 'idshare'] and run['depth'] in p['depths'] and run['seed'] in p['seeds']
    return c
...
def checks(p, output, run_key=None, gpu=False):
    routes = ([p['runs'][run_key]] if run_key else
              [dict(run_key=f'd{d}_{m}_s{s}', depth=d, method=m, seed=s)
               for s in p['seeds'] for d in p['depths'] for m in ['continuous', 'idshare']])
    ...
    for run in routes:
        ...
        if run['seed'] == 42:
            import depth_runtime_r01 as old
            assert c == old.config(p, run, target, local=True, train_only=True), 'R01 configuration drift'
        ...
        if run['method'] == 'idshare':
            ...
            assert q['initialization_seed'] == run['seed']
        ...
        local_counts[(run['seed'], run['depth'], run['method'])] = info['parameter_count']
        results.append(dict(run_key=run['run_key'], seed=run['seed'], **info, loss_first=losses[0], loss_last=losses[-1],
        ...
        for s in p['seeds']:
            for method in ['continuous', 'idshare']:
                for depth in p['new_depths']:
                    assert local_counts[(s, depth, method)] - local_counts[(s, 2, method)] == (depth-2)*p['parameters_per_block']
