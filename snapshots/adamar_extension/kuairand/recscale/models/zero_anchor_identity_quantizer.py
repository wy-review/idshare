...
class ZeroAnchorIdentityQuantizer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.codebook_size = int(cfg.get("codebook_size", 64))
        ...
        if self.codebook_size < 2:
            raise ValueError("zero-anchor codebook_size must include zero and nonzero")
    ...
    def deterministic_private_code_indices(
        ...
        return 1 + hashed.remainder(self.codebook_size - 1)

    def _first_touch_code_indices(self, ids: torch.Tensor) -> torch.Tensor:
        """Backward-compatible alias for the deterministic initializer."""
        return self.deterministic_private_code_indices(ids)
    ...
    def initialize_train_first_touch_rows(
        ...
        with torch.no_grad():
            for position, field in enumerate(self.identity_fields):
                ...
                code_indices = self._first_touch_code_indices(new_ids)
        ...
        *,
        ...
        if hard_indices is not None:
            ...
            if hard_indices.numel() and (
                ...
                or int(hard_indices.max().item()) >= self.codebook_size
            ):
    ...
    def _field_losses(
        ...
        zero_l2 = residuals.float().square().mean()
        return {
    ...
    def forward(
        ...
        for position, field in enumerate(self.identity_fields):
            ...
            if self.code_logit_bias_enabled:
                ...
                field_biases = self.code_logit_biases[position]
                if self.num_subspaces == 1:
            ...
            if regular_mask.any():
                ...
                if self.num_residual_levels > 1:
                    ...
                    if self.training:
                        with torch.no_grad():
                            self.assignment_counts[position].add_(
            ...
            if self.defer_cumulative_diagnostics:
                ...
                for axis in range(self.assignment_axis_count):
                    current = self._assignment_diagnostics(
                        self.assignment_counts[position, axis],
                        prefix=(
                ...
                diagnostics.update(
                    self._assignment_diagnostics(
                        self.assignment_counts[position],
                        prefix=f"za_{field}_cumulative",
                    )
                )
            else:
                ...
                for subspace in range(self.num_subspaces):
                    current = self._assignment_diagnostics(
                        self.assignment_counts[position, subspace],
                        prefix=(
    ...
    def get_center_displacement_by_assignment_frequency_report(self) -> dict:
        ...
        fields = []
        for position, field in enumerate(self.identity_fields):
            counts = self.assignment_counts[position, 1:].detach().cpu()
        ...
        for position, field in enumerate(self.identity_fields):
            if self.assignment_axis_count == 1:
                entries = [
                    (
                        None,
                        self.assignment_counts[position].detach().cpu(),
                        (
