#!/usr/bin/env python3
"""Run the frozen TAAC UnifiedMixer no-L2 carrier bridge."""
...
PROTOCOL = "taac_unifiedmixer_no_l2_bridge_r01"
...
def _synthetic_model(carrier, seed):
    params = {
...
def regression(matrix_path: Path) -> dict:
    ...
    for carrier in ("continuous", "idshare"):
        for seed in (2021, 42, 2024):
            ...
            model = _synthetic_model(carrier, seed)
            batch_size = 4
            batch = {
...
def train_one(config_dir: Path, expid: str, max_samples=None, emit_auc=True):
    ...
    if float(params.get("embedding_regularizer", -1)) != 0.0:
        raise RuntimeError("embedding L2 must remain disabled")
    if float(params.get("net_regularizer", -1)) != 0.0:
        raise RuntimeError("network L2 must remain disabled")
...
def smoke(matrix_path: Path, config_dir: Path, max_samples: int) -> dict:
    ...
    validate_matrix(matrix, matrix_path)
    expids = [
        matrix["runs"]["continuous_s2021"]["experiment_id"],
        matrix["runs"]["idshare_s2021"]["experiment_id"],
    ]
    rows = {}
    for expid in expids:
...
def formal(matrix_path: Path, config_dir: Path, run_key: str) -> dict:
    ...
    checks = {
        ...
        "no_l2": result["l2_coefficient"] == 0.0,
