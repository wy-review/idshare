...
class TAAC2025TimeDataset(BaseDataset):
    ...
    _shared_item_seen_mask = None
    _shared_item_seen_mask_key = None
    ...
    def __init__(self, config: dict, split: str = "train"):
        ...
        raw_item_values = [int(value) for value in indexer["i"].values()]
        raw_max_item_id = max(raw_item_values)
        if raw_max_item_id != len(raw_item_values):
            raise ValueError(
                "[TAAC2025-Time] item indexer must be contiguous 1..N for "
                "the shared identity remap"
            )

        self.exclude_target_item_from_sparse = bool(
            dc.get("exclude_target_item_from_sparse", False)
        )
        seen_mask_path = dc.get("train_input_seen_mask_path")
        self.shared_item_identity_remap_enabled = seen_mask_path is not None
        if self.exclude_target_item_from_sparse != self.shared_item_identity_remap_enabled:
            raise ValueError(
                "[TAAC2025-Time] train-input seen remapping and target sparse-path "
                "removal must be enabled together"
            )
        self._shared_item_seen_mask = None
        if self.shared_item_identity_remap_enabled:
            expected_sha = dc.get("train_input_seen_mask_sha256")
            if not isinstance(expected_sha, str) or len(expected_sha) != 64:
                raise ValueError(
                    "[TAAC2025-Time] train_input_seen_mask_sha256 is required"
                )
            mask_key = (os.path.realpath(seen_mask_path), expected_sha)
            if TAAC2025TimeDataset._shared_item_seen_mask_key != mask_key:
                digest = hashlib.sha256()
                with open(seen_mask_path, "rb") as handle:
                    for block in iter(lambda: handle.read(8 * 1024 * 1024), b""):
                        digest.update(block)
                if digest.hexdigest() != expected_sha:
                    raise ValueError(
                        "[TAAC2025-Time] train-input seen mask SHA256 mismatch"
                    )
                mask = np.load(seen_mask_path, mmap_mode="r", allow_pickle=False)
                if (
                    mask.ndim != 1
                    or mask.dtype != np.bool_
                    or mask.size != raw_max_item_id + 1
                    or bool(mask[0])
                ):
                    raise ValueError(
                        "[TAAC2025-Time] invalid train-input seen mask schema"
                    )
                TAAC2025TimeDataset._shared_item_seen_mask = mask
                TAAC2025TimeDataset._shared_item_seen_mask_key = mask_key
            self._shared_item_seen_mask = (
                TAAC2025TimeDataset._shared_item_seen_mask
            )
            # DIN allocates num_items + 1 rows. IDs 0..3 are special and
            # positive raw IDs map to raw_id + 3, so the maximum ID is N + 3.
            num_items = raw_max_item_id + 3
            dc["shared_item_identity_remap_enabled"] = True
            dc["shared_item_table_cardinality"] = raw_max_item_id + 4
            dc["shared_item_real_id_offset"] = 3
        else:
            num_items = len(indexer["i"]) + 1
            dc["shared_item_identity_remap_enabled"] = False
        ...
        if split == "train":
            ...
            sparse_cols = ["user_id"]
            cardinalities = [num_users]
            if not self.exclude_target_item_from_sparse:
                sparse_cols.append("target_item_id")
                cardinalities.append(num_items)
            ...
            fallback_cols = ["user_id"]
            fallback_cardinalities = [num_users]
            if not self.exclude_target_item_from_sparse:
                fallback_cols.append("target_item_id")
                fallback_cardinalities.append(num_items)
            sparse_cols = TAAC2025TimeDataset._shared_sparse_cols or fallback_cols
            cardinalities = (
                TAAC2025TimeDataset._shared_cardinalities
                or fallback_cardinalities
            )

        dc["sparse_cols"] = sparse_cols
        dc["cardinalities"] = cardinalities
        self._num_sparse = len(sparse_cols)
        base_sparse_count = 1 if self.exclude_target_item_from_sparse else 2
        self._has_side_features = len(sparse_cols) > base_sparse_count
        ...
        # Target identity is optionally removed from the sparse side path. Its
        # only carrier is then the shared target/sequence item table.
    ...
    def _remap_shared_item_ids(self, values: np.ndarray) -> np.ndarray:
        """Apply the frozen train-input support boundary to item IDs."""
        if not self.shared_item_identity_remap_enabled:
            return values
        values = np.asarray(values, dtype=np.int64)
        remapped = np.zeros_like(values)
        positive = values > 0
        in_range = positive & (values < self._shared_item_seen_mask.size)
        seen = np.zeros_like(positive)
        seen[in_range] = self._shared_item_seen_mask[values[in_range]]
        remapped[positive] = 1  # shared OOV
        remapped[seen] = values[seen] + 3
        if self.split == "train" and np.any(positive & ~seen):
            raise RuntimeError(
                "[TAAC2025-Time] train input contains an ID absent from the "
                "frozen train-input seen mask"
            )
        return remapped
    ...
    def __getitem__(self, idx):
        ...
        if self.shared_item_identity_remap_enabled:
            target = int(
                self._remap_shared_item_ids(
                    np.asarray([target], dtype=np.int64)
                )[0]
            )
            seq = self._remap_shared_item_ids(seq)
