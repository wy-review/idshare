...
@register_model
class HyFormerV2PropagateNSModel(RecModel):
    """
    HyFormer V2 with NS-token propagation experiment.

    New config option:
      - propagate_ns (bool): If True (default), ns_tokens are updated each layer
        and passed to the next (current behavior). If False, each layer receives
        the original ns_tokens without accumulated updates.
      - use_ns_in_boosting is forced to True in this model variant.
    ...
    """
    model_name = "hyformer_v2_propagate_ns"
    ...
    def __init__(self, config: dict):
        ...
        self.use_ns_in_boosting = True  # forced True for this experiment
        self.propagate_ns = mc.get("propagate_ns", True)  # True=逐层传递, False=每层用原始ns
    ...
    def forward(self, batch: dict) -> torch.Tensor:
        ...
        ns_orig = ns  # save original ns for non-propagate mode
        ...
        for layer_idx, block in enumerate(self.blocks):
            # use_ns_in_boosting is always True in this variant
            if self.propagate_ns:
                # ns is updated each layer and passed to the next
                queries, ns = block(
                ...
                # each layer receives the original ns (no accumulated updates)
                queries, _ = block(
                    queries, ns_orig, seq_kv, seq_mask,
                    seq2_kv=seq2_kv, seq2_mask=seq2_mask,
                    target_emb=target_condition,
                )
