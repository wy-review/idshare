...
class DINModel(RecModel):
    ...
    def __init__(self, config: dict):
        ...
        self.shared_item_quantization_deduplicate_occurrences = bool(
            mc.get("shared_item_quantization_deduplicate_occurrences", True)
        )
        if bool(zero_anchor_cfg.get("enabled", False)):
            if not bool(dc.get("shared_item_identity_remap_enabled", False)):
                raise ValueError(
                    "DIN zero-anchor identity quantization requires the frozen "
                    "shared target/sequence item remap"
                )
            if tuple(zero_anchor_cfg.get("identity_fields", ())) != (
                "shared_item_id",
            ):
                raise ValueError(
                    "DIN zero-anchor identity quantization must target only "
                    "shared_item_id"
                )
            if self.dual_seq:
                raise ValueError(
                    "DIN shared-item zero-anchor mode does not support a second "
                    "independent sequence table"
                )
            self.zero_anchor_identity_quantizer = ZeroAnchorIdentityQuantizer(
                config,
                cardinalities=[num_items + 1],
                field_names=["shared_item_id"],
                embedding_dim=emb_dim,
            )
            if self.zero_anchor_identity_quantizer.code_logit_bias_enabled:
                raise ValueError(
                    "DIN shared target/sequence quantization does not support "
                    "code-logit bias"
                )
    ...
    def _shared_item_embeddings(self, item_ids: torch.Tensor) -> torch.Tensor:
        """Look up target and sequence IDs through one matched carrier."""
        if self.zero_anchor_identity_quantizer is None:
            return self.item_emb(item_ids)
        original_shape = item_ids.shape
        flat_ids = item_ids.reshape(-1)
        if self.shared_item_quantization_deduplicate_occurrences:
            carrier_ids, inverse, occurrence_counts = torch.unique(
                flat_ids,
                sorted=True,
                return_inverse=True,
                return_counts=True,
        ...
        fields = self.item_emb(carrier_ids).unsqueeze(1)
        quantized = self.zero_anchor_identity_quantizer(
            fields,
            sparse_ids,
            row_weights=occurrence_counts,
        )
        embeddings = quantized[:, 0, :]
    ...
    def set_tau_for_step(self, current_step: int, total_steps: int) -> None:
        if self.zero_anchor_identity_quantizer is not None:
            self.zero_anchor_identity_quantizer.set_progress(
                current_step, total_steps
            )

    def resolve_full_table_l2_identity_weights(
        self,
        identity_fields,
    ) -> list[tuple[str, nn.Parameter]]:
        """Expose the one shared target/history item table to explicit L2."""
        fields = [str(field) for field in identity_fields]
        if fields != ["shared_item_id"]:
            raise ValueError(
                "DIN shared-item whole-table L2 must target only "
                "shared_item_id"
            )
        return [("shared_item_id", self.item_emb.weight)]

    def get_tokenizer_diagnostics(self) -> dict[str, float]:
        if self.zero_anchor_identity_quantizer is None:
            return {}
        return self.zero_anchor_identity_quantizer.get_diagnostics()

    def get_zero_anchor_identity_quantization_metadata(self):
        if self.zero_anchor_identity_quantizer is None:
            return None
        return self.zero_anchor_identity_quantizer.get_metadata()

    def get_zero_anchor_assignment_audit_report(self):
        if self.zero_anchor_identity_quantizer is None:
            return None
        return self.zero_anchor_identity_quantizer.get_assignment_audit_report()

    def get_zero_anchor_health_report(self, **thresholds):
        if self.zero_anchor_identity_quantizer is None:
            return None
        return self.zero_anchor_identity_quantizer.get_health_report(**thresholds)
    ...
    def forward(self, batch: dict) -> torch.Tensor:
        ...
        if seq is not None and target is not None:
            target_and_seq = torch.cat([target.unsqueeze(1), seq], dim=1)
            shared_embeddings = self._shared_item_embeddings(target_and_seq)
            target_emb = shared_embeddings[:, 0, :]
            seq_emb = shared_embeddings[:, 1:, :]
            ...
            target_emb = self._shared_item_embeddings(target)
            if self.use_item_side_info and "dense" in batch and batch["dense"] is not None:
        ...
        if self.zero_anchor_identity_quantizer is None:
            return logits
        aux_loss = self.zero_anchor_identity_quantizer.get_aux_loss()
        return (logits, aux_loss) if aux_loss is not None else logits
