...
def get_zero_anchor_identity_quantizer(model: nn.Module):
    """Return the enabled quantizer from either supported model layout."""
    raw_model = _raw_model(model)
    encoder = getattr(raw_model, "encoder", None)
    nested = getattr(encoder, "zero_anchor_identity_quantizer", None)
    ...
    if nested is not None and direct is not None and nested is not direct:
        raise ValueError(
            "model exposes two different zero-anchor identity quantizers"
        )
    return nested if nested is not None else direct


def _validate_resolved_identity_weights(
    weights,
    *,
    expected_fields: list[str] | tuple[str, ...],
) -> list[tuple[str, nn.Parameter]]:
    expected = [str(field) for field in expected_fields]
    try:
        resolved = list(weights)
    except TypeError as error:
        raise TypeError(
            "identity-table resolver must return an iterable of (field, weight)"
        ) from error
    fields = []
    checked = []
    for item in resolved:
        if not isinstance(item, (list, tuple)) or len(item) != 2:
            raise TypeError(
                "identity-table resolver entries must be (field, weight) pairs"
            )
        field, parameter = item
        field = str(field)
        if not isinstance(parameter, nn.Parameter) or parameter.ndim != 2:
            raise TypeError(
                f"identity table for {field!r} must be a rank-2 nn.Parameter"
            )
        fields.append(field)
        checked.append((field, parameter))
    if fields != expected:
        raise ValueError(
            "identity-table resolver returned fields in an unexpected order: "
            f"expected={expected}, actual={fields}"
        )
    return checked


def _resolve_model_owned_identity_weights(
    raw_model: nn.Module,
    *,
    ...
    resolver = getattr(
        raw_model,
        "resolve_full_table_l2_identity_weights",
        None,
    )
    if resolver is None:
        return None
    if not callable(resolver):
        raise TypeError(
            "model.resolve_full_table_l2_identity_weights must be callable"
        )
    return _validate_resolved_identity_weights(
        resolver(identity_fields),
...
def get_identity_embedding_weights(
    ...
    *,
    ...
    raw_model = _raw_model(model)
    model_owned = _resolve_model_owned_identity_weights(
        raw_model,
        identity_fields=identity_fields,
    )
    if model_owned is not None:
        return model_owned
    encoder = getattr(raw_model, "encoder", None)
    embeddings = getattr(
