...
class MultiTaskModel(BaseModel):
    ...
    def compile(self, optimizer, loss, lr, optimizer_embedding=None,
        ...
        """Multitask variant of BaseModel.compile.

        Delegates optimizer construction (including the dual sparse/dense
        path) to BaseModel.compile via a sentinel loss, then overwrites
        self.loss_fn with a per-task list. See BaseModel.compile docstring
        for the optimizer/optimizer_embedding/embedding_lr_decay contract.
        """
        # Reuse BaseModel's optimizer plumbing; loss arg here is irrelevant.
        super().compile(optimizer, "binary_cross_entropy", lr,
            ...
            for i in range(len(labels)):
                ...
                if self.writer is not None:
                    for k, v in val_logs.items():
                        self.writer.add_scalar(f"val/{labels[i]}/{k}", v, self._total_steps)
                for k, v in val_logs.items():
