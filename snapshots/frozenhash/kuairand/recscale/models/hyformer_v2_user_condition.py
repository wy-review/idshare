...
@register_model
class HyFormerV2UserCondModel(RecModel):
    """
    HyFormer V2 with user-token condition source experiment.

    Adds a new target_condition_source option: "item_user"
      - c = [target_emb; mean_pool(user_sparse_embeddings)]
      - User features are identified by prefix matching on sparse_cols names.

    Config:
      - target_condition_source: "item_user" (new option)
      - user_condition_prefix: str (default "user") — sparse_cols whose name
        starts with this prefix (case-insensitive) are treated as user features.
    ...
    """
    model_name = "hyformer_v2_user_cond"
    ...
    def __init__(self, config: dict):
        ...
        if self.target_condition_source not in ("item", "item_seq", "item_side", "item_side_seq", "item_user"):
            raise ValueError(
                "target_condition_source must be one of: item, item_seq, item_side, item_side_seq, item_user"
            )
        ...
        # --- item_user condition: identify user feature indices by prefix ---
        self.user_condition_prefix = mc.get("user_condition_prefix", "user")
        self.user_feature_indices = []  # will be populated after we know sparse_cols
        sparse_cols = dc.get("sparse_cols", [])
        prefix_lower = self.user_condition_prefix.lower()
        for idx, col_name in enumerate(sparse_cols):
            if col_name.lower().startswith(prefix_lower):
                self.user_feature_indices.append(idx)
        if self.target_condition_source == "item_user" and len(self.user_feature_indices) == 0:
            raise ValueError(
                f"target_condition_source='item_user' but no sparse_cols match "
                f"prefix '{self.user_condition_prefix}'. sparse_cols={sparse_cols}"
            )
        if self.target_condition_source == "item_user":
            print(f"[HyFormerV2UserCond] User feature indices ({len(self.user_feature_indices)}): "
                  f"{self.user_feature_indices} from cols: "
                  f"{[sparse_cols[i] for i in self.user_feature_indices]}")
        ...
        if self.target_condition_source != "item":
            ...
            if self.target_condition_source in ("item_seq", "item_side_seq"):
                ...
                if self.has_seq2:
                    target_condition_dim += hidden_dim
            if self.target_condition_source == "item_user":
                # mean-pooled user embeddings projected to hidden_dim
                target_condition_dim += hidden_dim
        ...
        # User condition projection: mean-pooled user sparse embeddings → hidden_dim
        if self.target_condition_source == "item_user":
            num_user_feats = len(self.user_feature_indices)
            self.user_cond_proj = nn.Linear(emb_dim, hidden_dim)
        else:
            self.user_cond_proj = None
        ...
        top_layers = []
        in_dim = num_queries * hidden_dim + hidden_dim + hidden_dim
    ...
    def forward(self, batch: dict) -> torch.Tensor:
        ...
        if self.target_condition_source != "item":
            ...
            if self.target_condition_source == "item_user":
                # Extract user-side sparse embeddings and mean-pool
                user_indices = self.user_feature_indices
                # sparse_tokens_for_qgen: (B, num_sparse, emb_dim)
                # Re-compute per-feature embeddings for user indices
                sparse_per_feat = self.sparse_arch.forward_per_feature(batch["sparse"])
                user_embs = sparse_per_feat[:, user_indices, :]  # (B, num_user, emb_dim)
                user_mean = user_embs.mean(dim=1)  # (B, emb_dim)
                user_repr = self.user_cond_proj(user_mean)  # (B, hidden_dim)
                cond_parts.append(user_repr)
        ...
        interest = self.target_attn(target_emb, seq_kv, seq_mask)
        interest2 = None
        if self.has_seq2:
