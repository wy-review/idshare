    ...
    with np.load(cache_path, allow_pickle=False) as payload:
        ...
        for field_position, field in enumerate(identity_fields):
            ...
            if quantized:
                ...
                tuple_count = (
                    quantizer.codebook_size**quantizer.num_subspaces
                )
                ...
                use_legacy_product_path = bool(
                    ...
                    and quantizer.codebook_size <= 1_024
                )
            ...
            for start in range(
                ...
                for subspace in range(quantizer.num_subspaces):
                    flat = (
                        buckets * quantizer.codebook_size
                        + indices[:, subspace]
                    )
                    center_counts[:, subspace].add_(
                        _deterministic_cpu_bincount(
                            flat,
                            minlength=(
                                bucket_count * quantizer.codebook_size
                            ),
                        )
                        .reshape(bucket_count, quantizer.codebook_size)
                        .double()
                    )
                tuple_ids = torch.zeros_like(indices[:, 0])
                for subspace in range(quantizer.num_subspaces):
                    tuple_ids = (
                        tuple_ids * quantizer.codebook_size
                        + indices[:, subspace]
                    )
                flat_tuples = (
                    buckets
            ...
            for label in FREQUENCY_BUCKETS:
                ...
                if self.quantized:
                    ...
                    if (
                        ...
                        hard_indices = (
                            quantizer.deterministic_private_code_indices(ids)
                        )
